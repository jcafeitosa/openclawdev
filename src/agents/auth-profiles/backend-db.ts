...
async function lazyDb() {
  ...
  const { authCredentials, authUsageStats, authStoreMeta } =
    await import("../../infra/database/drizzle-schema.js");
  ...
  async loadWithLock(params: {
    ...
    try {
      return await pgSql.begin(async (tx) => {
        ...
        // Cast needed: postgres TransactionSql is callable as tagged template but tsgo doesn't see the call signature
        await (tx as unknown as typeof pgSql)`SELECT pg_advisory_xact_lock(${LOCK_ID})`;
